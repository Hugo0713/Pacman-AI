...
def uniformCostSearch(problem: SearchProblem):
    ...
    while not pq.isEmpty():
        ...
        if state not in visited:
            ...
            for successor in successors:
                next_state, action, _ = successor
                pq.push((next_state, actions +[action]), problem.getCostOfActions(actions + [action]))
    return actions + [action]
    # util.raiseNotDefined()
        ...
        if state not in visited:
            ...
            for successor in successors:
                ...
                if (next_state not in visited):
                    ...
                    newCost = problem.getCostOfActions(newActions) + heuristic(next_state, problem)
                    pq.push((next_state, newActions), newCost)
    return actions + [action]
    # util.raiseNotDefined()
