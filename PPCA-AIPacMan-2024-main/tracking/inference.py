...
def constructBayesNet(gameState: hunters.GameState):
    ...
    MAX_DISTANCE = X_RANGE + Y_RANGE  # 最大可能的曼哈顿距离
    variableDomainsDict[OBS0] = [i for i in range(MAX_DISTANCE + MAX_NOISE - 1)]
    variableDomainsDict[OBS1] = [i for i in range(MAX_DISTANCE + MAX_NOISE - 1)]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        "*** YOUR CODE HERE ***"
        # 计算每个位置应该有多少个粒子
        numPos = len(self.legalPositions)  # 合法位置的数量
        numEachPos = self.numParticles // numPos  # 每个位置基本分配的粒子数
        extraParticles = self.numParticles % numPos  # 不能均匀分配的额外粒子数
        
        # 首先为每个位置分配基本的粒子数
        for pos in self.legalPositions:
            for _ in range(numEachPos):
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            newParticles.append(self.getPositionDistribution(gameState, oldPos).sample())
