...
def sentence1() -> Expr:
    ...
    not_A_iff_not_B_or_C = ~Expr('A') % ((~Expr('B')) | Expr('C'))
    not_A_or_not_B_or_C = disjoin([(~Expr('A')), (~Expr('B')), Expr('C')])
...
def sentence2() -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    C_iff_B_or_D = Expr('C') % (Expr('B') | Expr('D'))
    A_implies_not_B_and_not_D = Expr('A') >> (~Expr('B') & ~Expr('D'))
    not_B_and_not_C_implies_A = ~(Expr('B') & ~Expr('C')) >> Expr('A')
    not_D_implies_C = ~Expr('D') >> Expr('C')
...
def sentence3() -> Expr:
    ...
    PacmanAlive_1_iff = PacmanAlive_1 % disjoin([(PacmanAlive_0 & ~PacmanKilled_0), (~PacmanAlive_0 & PacmanBorn_0)])
    PacmanAlive_0_xor_PacmanBorn_0 = ~(PacmanAlive_0 & PacmanBorn_0)
    PacmanBorn_0 = PropSymbolExpr('PacmanBorn_0')
    return conjoin([PacmanAlive_1_iff, PacmanAlive_0_xor_PacmanBorn_0, PacmanBorn_0])
    # util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    # model_premise = findModel(premise)
    # model_conclusion = findModel(conclusion)
    # if model_premise == False or model_conclusion == False:
    #     return False
    # for key in model_conclusion.keys():
    #     if model_conclusion[key] != model_premise[key]:
    #         return False
    # return True
    negation_conclusion = ~conclusion
    return findModel(conjoin([premise, negation_conclusion])) == False
...
def positionLogicPlan(problem) -> List:
    ...
    for timestep in range(100):
        ...
        KB.append(exactlyOne([PropSymbolExpr(pacman_str, x, y, time=timestep) for x, y in non_wall_coords]))
...
def foodLogicPlan(problem) -> List:
    ...
    for timestep in range(100):
        ...
        if findModel(conjoin(KB) & ~atLeastOne([PropSymbolExpr(food_str, x, y, time= timestep) for x, y in food])) != False:
            return extractActionSequence(findModel(conjoin(KB) & ~atLeastOne([PropSymbolExpr(food_str, x, y, time= timestep) for x, y in food])), actions)
    return []
    # util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
